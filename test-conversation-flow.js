...
async function testConversationFlow() {
  ...
  let response = await replyFromLLM(history, 'Hello');
  ...
  response = await replyFromLLM(history, 'What services do you offer?');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
  ...
  response = await replyFromLLM(history, 'I want a New Customer Discovery Call');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
  ...
  response = await replyFromLLM(history, 'My name is John Smith');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
  ...
  response = await replyFromLLM(history, 'My email is john@example.com');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
  ...
  response = await replyFromLLM(history, 'Yes, that is correct');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
  ...
  response = await replyFromLLM(history, 'Actually, I need to change my email');
  console.log('Bot:', response.text);
  history = response.history;
  console.log('');
}
